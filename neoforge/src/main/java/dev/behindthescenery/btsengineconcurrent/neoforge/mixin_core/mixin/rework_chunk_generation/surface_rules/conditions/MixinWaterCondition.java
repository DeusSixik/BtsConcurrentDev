...
@Mixin(targets = "net.minecraft.world.level.levelgen.SurfaceRules$WaterCondition")
public class MixinWaterCondition {

    @Unique
    private final ThreadLocal<SurfaceRuleConditions.WaterCondition> bts$local_condition = new ThreadLocal<>();
    ...
    public SurfaceRules.Condition apply(final SurfaceRules.Context context) {
        SurfaceRuleConditions.WaterCondition condition = bts$local_condition.get();

        if(condition == null)
            bts$local_condition.set(condition = new SurfaceRuleConditions.WaterCondition(ReflectionsUtils.cast(this), context));
        else {
