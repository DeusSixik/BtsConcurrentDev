...
public class MixinStoneDepthCheck {

    @Unique
    private final ThreadLocal<SurfaceRuleConditions.StoneDepthCondition> bts$local_condition = new ThreadLocal<>();
    ...
    public SurfaceRules.Condition apply(final SurfaceRules.Context context) {
        SurfaceRuleConditions.StoneDepthCondition condition = bts$local_condition.get();

        if(condition == null)
            bts$local_condition.set(condition = new SurfaceRuleConditions.StoneDepthCondition(ReflectionsUtils.cast(this), context));
        else {
