...
public class MixinYConditionSource {

    @Unique
    private final ThreadLocal<SurfaceRuleConditions.YCondition> bts$local_condition = new ThreadLocal<>();
    ...
    public SurfaceRules.Condition apply(final SurfaceRules.Context context) {
        SurfaceRuleConditions.YCondition condition = bts$local_condition.get();

        if(condition == null)
            bts$local_condition.set(condition = new SurfaceRuleConditions.YCondition(ReflectionsUtils.cast(this), context));
        else {
