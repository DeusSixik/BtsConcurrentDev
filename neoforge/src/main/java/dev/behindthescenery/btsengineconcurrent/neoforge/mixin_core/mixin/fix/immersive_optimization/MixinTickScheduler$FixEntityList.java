package dev.behindthescenery.btsengineconcurrent.neoforge.mixin_core.mixin.fix.immersive_optimization;
...
public abstract class MixinTickScheduler$FixEntityList {
    ...
    void tickLevel(ServerLevel level) {
        TickScheduler.LevelData data = (TickScheduler.LevelData)this.levelData.computeIfAbsent(level.dimension().location(), TickScheduler.LevelData::new);
        ...
        if (!Config.getInstance().enableEntities) {
            ...
            ((ServerLevelAccessor)level).getEntityTickList().forEach((entity) -> {
                if (entity != null) {
                    ...
                    TickScheduler.Stats var10000 = data.stats;
                    var10000.tickRate += (double)1.0F / (double)Math.max(1, priority);
                    ++data.stats.entities;
                }
